...
class ObdDiscoveryService {
  ...
  Future<String?> _tryUdpDiscovery() async {
    try {
      ...
      socket.listen((event) {
        if (event == RawSocketEvent.read) {
          ...
          if (datagram != null) {
            ...
            debugPrint('[UDP] <<< RECEIVED: $response');
            ...
            } else if (response.contains('I_AM_OBD')) {
              // Optional: respond with acknowledgment
              socket.send(utf8.encode("OBD_HERE:SIMULATOR_IP"), datagram.address, datagram.port);
            }
          }
        }
      });
